...
type Schema = 'anthropic' | 'openai' | 'ollama';
...
export function resolveLlmConfig(config: AgentServiceConfig): LLMProvider {
  ...
  let schema: Schema = config.schema as Schema | undefined ?? 'openai';
  ...
  if (legacyProvider && !config.schema) {
    ...
    schema = legacyProvider === 'anthropic' ? 'anthropic'
      : legacyProvider === 'ollama' ? 'ollama'
        : 'openai';
  }
